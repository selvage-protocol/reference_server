#!/usr/bin/env python3
"""Deploy the Pi demo to two digest-pinned images.

Installed by hand as `/usr/local/sbin/selvage-pi-deploy`, root-owned and mode
0755, and the only command the `deployci` user on that box may run through sudo
(`packaging/pi/deployci.sudoers`, whose line ends in `""` so that sudo permits no
argument at all rather than any). That confinement is why this takes **no
arguments**: an argument to a script that reaches `docker compose` is a root
shell with extra steps, and leaving the argument list to this program alone would
put the entire bound inside one `sys.argv` check. The request arrives on stdin
instead — `COMPOSE_SHA256` plus one or both image references — and every value it
...
* it cannot edit the shape: the compose file is *verified* against the hash the
  ...
  a caller cannot add a port, publish another address, drop a capability or
  change a command.

A run that fails leaves the box as it found it. The references it is deploying go
into `/root/selvage-compose/.env.deploy`, which is what `pull` and `up` read;
`/root/selvage-compose/.env`, the intent for the next `up -d`, is written only
once the containers are up and converged on the digests that were asked for. One
bad release therefore cannot leave a box that reproduces it on the next `up -d`,
hand or automated.
...
# may name, and which compose service it lands on. There is no third: this box
# has no front, because the tailnet is the boundary.
PINS = {
    "SELVAGED_IMAGE": ("selvaged", "selvaged"),
    "SELVAGE_WEB_IMAGE": ("selvage-web", "selvage-web"),
}
SERVICES = ("selvaged", "selvage-web")
...
def render_env(pins: dict) -> str:
    return (
        "# Generated by /usr/local/sbin/selvage-pi-deploy: the two published images the\n"
        "# next `docker compose up -d` runs. Digests, not tags, because a tag is a name\n"
        "# the publisher can repoint. The file this replaced is .env.prev; the explained\n"
        "# copy is .env.example, and README.md beside it owns the shape.\n"
        + "".join(f"{key}={pins[key]}\n" for key in PINS)
    )
...
def compose_argv(env_file: Path, *arguments: str) -> list[str]:
    ...
    The compose file's own directory is the project directory, and its `name:` is
    what names the project, so nothing here needs `-p` or `--project-directory`.
    `--env-file` is explicit and is the caller's: a deploy reads the staged file,
    and everything else reads the persistent one.
    """
    return [
        "docker",
        "compose",
...
def compose(env_file: Path, *arguments: str) -> subprocess.CompletedProcess:
    return subprocess.run(compose_argv(env_file, *arguments), text=True, check=False)
...
def main(argv: list[str]) -> int:
    if len(argv) != 1:
        print("selvage-pi-deploy takes no arguments; the request arrives on stdin", file=sys.stderr)
        return 2
    if os.geteuid() != 0:
        print("selvage-pi-deploy must run as root", file=sys.stderr)
        return 2
